    ...
    def successor(self, current_state):
        for first_edge, second_edge in current_state.edges:
            if current_state.sites[first_edge] != current_state.sites[second_edge]:
                ...
                new_state.sites[first_edge], new_state.sites[second_edge] = new_state.sites[second_edge], \
                new_state.sites[first_edge]
...
def randomized_maxvalue(problem, limit=100):
    ...
    for step in range(limit):
        ...
        for successor in current.expand():
            if len(max_successor) < 5:
                max_successor.append(successor)
            else:
                for i in range(len(max_successor)):
                    if successor.value() < max_successor[i].value():
                        max_successor[i] = successor
                        break

        current = random.choice(max_successor)
